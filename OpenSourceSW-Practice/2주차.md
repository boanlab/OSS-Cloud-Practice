...
# 2. [PDF 10쪽] boanlab 이미지를 활용한 복합 옵션 예시
# 🚨 주의: 끝에 'my-command' 파일이 없어서 에러(executable file not found)가 발생하는 것이 정상인 학습용 예시입니다.
docker run -i -t --rm -d --name hello-world -p 80:80 -v /opt/hello:/world boanlab/hello-world:latest my-command

# 3. Nginx 웹 서버 컨테이너 백그라운드 실행 (이것이 메인 실습입니다!)
